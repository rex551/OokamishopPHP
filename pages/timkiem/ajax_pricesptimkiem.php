...
{
    $sql = "SELECT * FROM tbl_sanpham,tbl_danhmuc WHERE tbl_sanpham.id_danhmuc=tbl_danhmuc.id_danhmuc AND tbl_sanpham.tensanpham LIKE '%".$_POST["key"]."%' 0 AND tinhtrang=1 ORDER BY giasp DESC LIMIT 40" ;
    $result = mysqli_query($mysqli,$sql);

    if(isset($_POST["price"]) && !empty($_POST["price"])){
        $sql = "SELECT * FROM tbl_sanpham,tbl_danhmuc WHERE tbl_sanpham.id_danhmuc=tbl_danhmuc.id_danhmuc AND tbl_sanpham.tensanpham LIKE '%".$_POST["key"]."%' AND tinhtrang=1 AND  giasp BETWEEN  0 AND '".$_POST["price"]."' LIMIT 40 ";
        $result = mysqli_query($mysqli,$sql);
    }
    $output = '';
